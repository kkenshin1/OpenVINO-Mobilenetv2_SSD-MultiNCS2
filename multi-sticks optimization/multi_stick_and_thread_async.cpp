...
const string label_file = "/home/jmj/project/NCS_optimization/labels.txt" ;
bool flag = true ;
int ncsnum ;                            //计算棒数量
queue <Mat> frame_que ;                 //图像队列
queue <const float * > result_que ;     //结果队列
ostringstream detecfps ;                //推断帧率
ostringstream reappearfps ;             //摄像头帧率
ostringstream ncs_number ;
ostringstream request_number ;
mutex mtx1,mtx2 ;                       //全局互斥锁 1-frame_que 2-result_que
    ...
    if (result == NULL){
        ...
        for (int i=0; i < 100 ; i++){
            ...
            if(confidence > 0.5){
                ...
                putText(image, labels[label]+conf.str(), Point2f(xmin,ymin-5),FONT_HERSHEY_TRIPLEX,1,Scalar(0,0,255)) ;
                rectangle(image,Point2f(xmin,ymin),Point2f(xmax,ymax),Scalar(0,0,255),2) ;
            }
        }
...
void camera(){
    ...
    request_number<<"eachNCS request number: 2" ;
...
void inference(Core ie , string device , int ncsid){
    ...
    while(flag){
        ...
        mtx1.lock() ;
        next_frame = frame_que.front() ;
        frame_que.pop() ;
        mtx1.unlock() ;

        if(isFirstFrame){       //第一帧单独执行
            ...
            if(OK == infer_request_curr->Wait(IInferRequest::WaitMode::RESULT_READY)){
                const float *detection = infer_request_curr->GetBlob(outputName)->buffer().as<PrecisionTrait<Precision::FP32>::value_type*>() ;
                mtx2.lock() ;
