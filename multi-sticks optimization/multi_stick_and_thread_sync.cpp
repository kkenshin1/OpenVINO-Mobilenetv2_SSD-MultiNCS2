...
void camera(){
    ...
    request_number<<"eachNCS request number: 1" ;
...
void inference(Core ie , string device , int ncsid){
    ...
    //设置推断请求
    InferRequest::Ptr infer_request = exctnet.CreateInferRequestPtr() ;
    ...
    while(flag){
        ...
        mtx1.lock() ;
        frame = frame_que.front() ;
        frame_que.pop() ;
        mtx1.unlock() ;
        Blob::Ptr frameBlob = infer_request->GetBlob(imageInputName) ;
        matU8ToBlob<uint8_t>(frame,frameBlob) ;
        infer_request->StartAsync() ;
        if(OK==infer_request->Wait(IInferRequest::WaitMode::RESULT_READY)) {
            const float *detection = infer_request->GetBlob(outputName)->buffer().as<PrecisionTrait<Precision::FP32>::value_type*>() ;
            mtx2.lock() ;
